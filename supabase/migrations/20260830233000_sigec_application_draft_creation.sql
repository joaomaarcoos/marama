...
begin
  ...
  if target_id is not null then
    ...
    select application.id, application.application_state
      into target_id, target_state
...
revoke insert on public.sigec_applications from authenticated;
...
comment on function public.sigec_create_application_draft(uuid) is
  'Creates one idempotent candidate-owned application draft after server-verified profile and process eligibility checks.';
