-- P1-01 correction: GoTrue persists Admin API app_metadata after INSERT.
-- Candidate signup is therefore finalized after insert using a non-privileged marker.
...
create trigger sigec_finalize_candidate_signup
after insert on auth.users
for each row execute function private.sigec_finalize_candidate_signup();

comment on function private.sigec_finalize_candidate_signup() is
  'Assigns only candidato and creates its profile after an explicitly marked public signup; internal Admin API users are unaffected.';
