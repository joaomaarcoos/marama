...
**Fase atual:** Fase 3 em implementação na `master`; o ClamAV 1.5.4 está operacional em produção no stack `mara`, sem porta publicada e conectado à rede externa existente; Gate P2 permanece adiado e Gate P3 ainda aguarda P3-06 e os smokes de arquivo limpo/EICAR
...
- [x] SIGEC-P3-05 — Adicionar varredura antimalware e quarentena antes de disponibilizar documento à comissão. Concluído em 30/08/2026 com ClamAV 1.5.4 fixado como serviço interno sem porta publicada, protocolo `INSTREAM`, timeout e erros fechados, resultados vinculados ao hash, tentativas auditadas e rota gerencial de reprocessamento com nova conferência SHA-256. Somente `malware_status = clean` libera leitura no Storage, inclusive para o próprio candidato; `pending`, `infected` e `error` permanecem em quarentena. O runtime Docker foi elevado a Node 22, suportado pelo Supabase JS. O serviço foi incluído manualmente no stack `mara` pela definição do Portainer e confirmado operacional em produção. **Smoke funcional pendente:** validar arquivo limpo e EICAR antes de homologar o Gate P3.
...
- [ ] SIGEC-P4-05 — Gerar protocolo imutável com data, versão do edital, respostas, opções, consentimentos e hashes dos documentos.
- [ ] SIGEC-P4-06 — Permitir correção até o encerramento com histórico; para edital de inscrição única, manter apenas a última versão válida.
- [ ] SIGEC-P4-07 — Impedir alterações após o prazo, exceto diligência administrativa explicitamente aberta.
