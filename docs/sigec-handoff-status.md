...
**Fase atual:** Fase 7 iniciada na branch `codex/sigec-p7-01-janela-recursos`; `SIGEC-P7-01` concluída; itens P6-05 a P6-08 permanecem aguardando confirmações normativas
**Progresso auditado:** 53 de 88 tarefas concluídas; 35 pendentes
**Última auditoria automática:** aprovada em 03/09/2026, sem achados locais ou remotos acionáveis
...
**Gate P4:** aprovado em 31/08/2026. Chamadas realmente concorrentes para criar rascunho, substituir preferências/respostas, enviar, abrir correção e reenviar preservam atomicidade e idempotência: existe uma candidatura, um protocolo por versão, uma versão vigente e um evento de negócio por transição. Na diligência, uma resposta concorrente conclui a solicitação e a repetida falha fechada sem duplicar histórico.

### Fase 5 — Triagem e análise administrativa

- [x] SIGEC-P5-01 — Criar lista paginada com filtros por processo, município, curso, modalidade, concorrência, status e pendência. Concluída em 01/09/2026 com paginação server-side de 25 itens, busca por nome/protocolo, filtros combináveis, projeção operacional sem CPF, WhatsApp, endereço, respostas ou arquivos e acesso exclusivo de `admin`/`gerente`.
- [x] SIGEC-P5-02 — Criar tela de candidatura com documentos, respostas, versões, consentimentos e histórico. Concluída em 01/09/2026 com detalhe responsivo em modo somente leitura, versões de documentos e protocolos, aceites sem fingerprints e histórico de etapas. A projeção administrativa omite CPF, contato, endereço, caminho/nome original de arquivo, snapshot bruto e metadados de auditoria.
- [x] SIGEC-P5-03 — Validar/rejeitar documentos com motivo público obrigatório e nota interna separada. Concluída em 01/09/2026 com leitura protegida do arquivo atual limpo, conferência de tamanho/hash antes da entrega, revisão atômica somente em candidatura enviada, motivo público obrigatório na rejeição, nota interna separada e histórico imutável.
- [x] SIGEC-P5-04 — Implementar diligência com campos autorizados, prazo e trilha de auditoria. Concluída em 01/09/2026 com uma única solicitação ativa por candidatura, seleção exata de perguntas/documentos do processo, prazo limitado, criação/aceite/cancelamento somente por `admin`/`gerente`, mensagem pública de encerramento e auditoria sem conteúdo das mensagens.
- [x] SIGEC-P5-05 — Bloquear avanço enquanto houver documento obrigatório pendente/rejeitado ou diligência aberta. Concluída em 01/09/2026 com gate transacional no banco, contagem segura de bloqueios, exigência de documento atual aprovado, aceite formal de diligência respondida, transição configurada, motivo público e histórico/auditoria com ator, instante, versão e campo alterado.
- [x] SIGEC-P5-06 — Implementar motivos de desclassificação conforme itens 6.1.1 a 6.1.9 do edital. Concluída em 01/09/2026 com catálogo versionado dos nove motivos históricos, inicialmente pendente e inutilizável até confirmação normativa explícita. A decisão exige `admin`/`gerente`, candidatura enviada, motivo do catálogo confirmado do mesmo processo e transição ativa para `desclassificado`; mensagem pública e nota interna permanecem separadas. O candidato consulta somente seu motivo e sua mensagem por função privada com verificação de propriedade.
- [x] SIGEC-P5-07 — Garantir que atendente não altere análise, nota, classificação ou convocação sem autorização expressa. Concluída em 01/09/2026 com gate remoto negativo: `atendente` não é reconhecido como equipe SIGEC, não acessa as rotas administrativas nem lê candidaturas/notas/rankings/convocações, e não consegue alterar análise, avanço, desclassificação, pontuação, classificação, aprovação/publicação de ranking ou convocação. Qualquer permissão futura exigirá mudança explícita e auditável da matriz de papéis.

**Gate P5:** aprovado em 01/09/2026. Toda decisão identifica ator, instante, motivo, versão analisada e dados alterados; mensagens públicas e notas internas permanecem separadas, e o papel `atendente` falha fechado em todas as mutações da fase.
...
- [x] SIGEC-P6-02 — Implementar experiência por faixas, máximo 40, com regra explícita para sobreposição de vínculos. Concluída em 01/09/2026 com revisão append-only de cada vínculo docente, exigência de comprovante atual aprovado, snapshot das datas e corte na data de envio. O banco une intervalos aprovados sobrepostos, converte dias únicos em meses completos de 30 dias e aplica 0/5/10/20/30/40, mantendo 40 acima de 60 meses. Painel administrativo responsivo exibe o total e permite aprovar ou rejeitar cada vínculo com motivo público.
- [x] SIGEC-P6-03 — Implementar no motor de avaliação a rubrica aprovada de produção acadêmica e desenvolvimento profissional, com teto de 30 pontos, comprovação, limites por categoria e trilha de auditoria. Concluída em 03/09/2026 com revisão append-only por comprovante, categorias e tetos 10/5/6/4/5, teto global 30, pertinência obrigatória, bloqueio de comprovante usado como requisito e justificativas separadas. O mesmo comprovante possui uma única classificação vigente e pode ser reclassificado sem dupla contagem. A validade sugerida de cinco anos continua pendente e não foi imposta.
- [x] SIGEC-P6-04 — Calcular nota total no servidor/banco e impedir pontos acima do máximo por critério. Concluída em 03/09/2026 com consolidação atômica server-only, limites 30/40/30 e total 100, snapshot append-only encadeado, versão do algoritmo e atualização da nota operacional. O painel mostra os três componentes, divergência pendente e exige ação explícita para gerar nova versão.
...
- [x] SIGEC-P6-09 — Exigir revisão humana e dupla confirmação antes de publicar ou substituir resultado oficial. Concluída em 03/09/2026 com duas confirmações imutáveis de pessoas distintas, restritas a `admin`/`gerente`, operações server-only e confirmação explícita na interface. Somente o snapshot oficial congelado mais recente pode ser revisado ou publicado; substituições formam uma cadeia append-only e preservam a publicação anterior. O fluxo permanece vazio e bloqueado enquanto P6-05/P6-06 não produzirem classificação oficial.

**Gate P6:** amostras conhecidas cobrem todos os empates, limites de pontuação, cotas, reversões e mudanças após recurso.

### Fase 7 — Recursos

- [x] SIGEC-P7-01 — Abrir janela de 24 horas a partir de 00h00 do dia seguinte à publicação preliminar. Concluída em 03/09/2026 com janela imutável criada automaticamente para cada publicação ou substituição preliminar, usando `America/Sao_Paulo`. O banco bloqueia recursos antes de 00h00 do dia seguinte e a partir do fechamento exclusivo 24 horas depois; a área do candidato mostra início, fim e estado em linguagem simples.
...
| 01/09/2026 | SIGEC-P5-01 | migration `sigec_admin_application_list`; rota `/sigec-candidaturas`; filtros e lista responsiva; teste remoto e matriz HTTP por papel | RPC `SECURITY INVOKER` disponível somente ao backend lista dados estritamente operacionais, com paginação e filtros aplicados no banco. Vinte e cinco controles remotos validaram paginação, projeção, filtros, busca, papéis e rollback; 20 cenários HTTP confirmaram acesso somente de admin/gerente; 143 controles da aplicação, 41/41 tabelas RLS, verificação remota, TypeScript, build de 58 páginas e Advisors sem achados SIGEC acionáveis foram aprovados. Nenhum deploy foi executado; publicação permanece manual. |
| 01/09/2026 | SIGEC-P5-02 | migration `sigec_admin_application_detail`; rota `/sigec-candidaturas/[id]`; detalhe administrativo responsivo | leitura `SECURITY INVOKER` somente no backend reúne candidatura, opções, respostas, versões documentais, protocolos, aceites e andamento sem expor os campos sensíveis excluídos do contrato. Dezenove controles remotos validaram conteúdo, versões, papéis, inexistência, privilégios e rollback; 23 cenários HTTP confirmaram isolamento inclusive na rota dinâmica; 149 controles da aplicação, 41/41 tabelas RLS, verificação remota, TypeScript, build de 58 páginas e Advisors sem achados SIGEC acionáveis foram aprovados. Nenhuma ação de análise foi antecipada e nenhum deploy foi executado. |
| 01/09/2026 | SIGEC-P5-03 | migrations `sigec_document_review_workflow` e `sigec_document_review_submitted_gate`; API protegida de leitura; controles de análise | somente a versão atual, tecnicamente validada, limpa e pertencente a candidatura enviada pode ser aberta ou analisada. A entrega revalida tamanho e SHA-256, usa nome genérico e proíbe cache. Rejeição exige motivo público; nota interna fica em coluna/tabela server-only separada; decisões são append-only e a auditoria registra apenas presença, não conteúdo. Vinte controles remotos, 27 cenários HTTP, 158 controles da aplicação, 42/42 tabelas RLS, verificação remota, TypeScript, build de 58 páginas e Advisors sem achados SIGEC acionáveis foram aprovados. Nenhum deploy foi executado. |
| 01/09/2026 | SIGEC-P5-04 | migration `sigec_information_request_management`; gestão responsiva na análise da candidatura; RPCs transacionais de abertura e encerramento | somente `admin`/`gerente` abre solicitação para candidatura enviada com snapshot, campos pertencentes ao processo e prazo futuro de até 365 dias. Índice parcial impede duas diligências ativas; resposta só pode ser aceita após o candidato concluir os itens, enquanto cancelamento exige justificativa pública. Vinte e dois controles remotos, regressão de prontidão com 46 controles e gate concorrente com 28 controles passaram sequencialmente; 165 controles da aplicação/SQL, 42/42 tabelas RLS, verificação remota, TypeScript, build de 58 páginas e Advisors sem achados acionáveis foram aprovados. A regressão antiga foi ajustada para encerrar formalmente a diligência respondida antes de abrir outra. Nenhum deploy foi executado; publicação permanece manual. |
| 01/09/2026 | SIGEC-P5-05 | migration `sigec_application_advancement_gate`; painel responsivo de avanço; RPCs de prontidão e transição | documento obrigatório ausente, pendente ou rejeitado bloqueia a mudança; diligência aberta ou respondida também bloqueia até aceite/cancelamento formal. A transição precisa existir no fluxo ativo e toda mudança exige mensagem pública. Vinte controles remotos validaram papéis, cada bloqueio, liberação, destino, atomicidade, histórico, versão analisada, campos alterados, privilégios e rollback. Auditorias aprovaram 170 controles da aplicação/SQL, 42/42 tabelas RLS, verificação remota, TypeScript, build de 58 páginas e zero Advisors acionáveis. Nenhum deploy foi executado; publicação permanece manual. |
...
| 01/09/2026 | SIGEC-P6-02 | migration `sigec_experience_band_scoring`; evidências versionadas por vínculo; painel administrativo | o cálculo usa somente experiências docentes aprovadas e comprovadas, congela as datas analisadas, corta vínculos na data de envio e une sobreposições antes de converter dias únicos em meses de 30 dias. Todas as bordas de 0 a 80 meses, a sobreposição, papéis, propriedade, imutabilidade e ausência do motivo na auditoria passaram em 31 controles remotos com rollback e limpeza. Auditorias aprovaram 192 controles da aplicação, 48/48 tabelas RLS, verificador remoto, TypeScript, build de 58 páginas e zero Advisors acionáveis. Nenhum deploy foi executado; publicação permanece manual. |
| 03/09/2026 | SIGEC-P6-03 | migration `sigec_academic_production_scoring`; motor de rubrica; painel administrativo responsivo | artigos, livros/capítulos, produção técnica, apresentações e formação continuada respeitam tetos 10/5/6/4/5 e total 30. Somente comprovante atual aprovado e relacionado à vaga pontua; reutilização como requisito é bloqueada e reclassificação preserva o histórico sem dupla contagem. Vinte e seis controles remotos passaram com rollback/limpeza; auditorias aprovaram 197 controles da aplicação, 49/49 tabelas RLS, verificador remoto, TypeScript, build de 58 páginas e zero Advisors acionáveis. Nenhum deploy foi executado; publicação permanece manual. |
| 03/09/2026 | SIGEC-P6-04 | migration `sigec_consolidated_score_snapshot`; nota operacional; painel de consolidação | a nota é calculada exclusivamente no banco a partir dos três motores auditados, com tetos 30/40/30 e total 100. Cada recálculo cria snapshot imutável encadeado com algoritmo `sigec-score-v1` e detalhes não sensíveis, atualizando `score_total` na mesma transação. Dezesseis controles remotos passaram com rollback/limpeza; auditorias aprovaram 201 controles da aplicação, 50/50 tabelas RLS, verificador remoto, TypeScript, build de 58 páginas e zero Advisors acionáveis. Nenhum deploy foi executado; publicação permanece manual. |
| 03/09/2026 | SIGEC-P6-09 | migrations `sigec_ranking_publication_four_eyes`, `sigec_ranking_actor_validation_fix` e `sigec_ranking_approval_count_fix`; painel de revisão/publicação; RPCs server-only | a auditoria dinâmica encontrou e corrigiu de forma incremental dois conflitos de permissão do schema Auth, preservando o histórico já aplicado. Atendente não confirma nem publica; duas pessoas diferentes são obrigatórias; versão antiga é bloqueada; republicação encadeia e preserva a anterior; ações geram auditoria. Vinte e quatro controles remotos passaram com rollback/limpeza; auditorias aprovaram 206 controles da aplicação e 50/50 tabelas RLS, verificador remoto, build de 58 páginas e zero Advisors acionáveis. Nenhum deploy foi executado; publicação permanece manual. |
| 03/09/2026 | SIGEC-P7-01 | migrations `sigec_appeal_window_from_preliminary_publication`, `sigec_appeal_window_timestamp_fix` e `sigec_appeal_window_trigger_privilege_fix`; prazo na área do candidato; gate de banco | cada publicação preliminar agenda uma janela append-only das 00h00 do dia seguinte até 00h00 subsequente em `America/Sao_Paulo`; resultado final não abre janela e republicação cria nova versão. A validação detectou um alias reservado e os testes encontraram conflitos de tipo/permissão, todos corrigidos incrementalmente antes do fechamento. Vinte e sete controles remotos passaram com rollback/limpeza; nenhum deploy foi executado e nenhuma publicação real foi criada. |

## 9. Próxima ação recomendada

Prosseguir por `SIGEC-P7-02`, restringindo o recurso à própria candidatura e aos objetos permitidos, sem antecipar os objetos normativos ainda não confirmados. Manter `SIGEC-P6-05` pendente até a confirmação da ordem oficial dos desempates; P6-06, P6-07 e P6-08 continuam fechadas porque classificação, listas e cotas dependem dessas decisões. Para fechar posteriormente o Gate P2, ainda será necessário revisar a prévia real, importar as 364 linhas prontas, corrigir ou excluir as 25 pendentes e finalizar as decisões normativas. A criação/publicação do primeiro processo seletivo real dentro do SIGEC permanece manual e condicionada aos gates.
