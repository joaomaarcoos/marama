...
**Fase atual:** `SIGEC-P5-02` concluída na branch `codex/sigec-p5-02-detalhe-candidatura`; próxima implementação é `SIGEC-P5-03`; Gate P2 permanece adiado pelas confirmações normativas e pela importação oficial
**Progresso auditado:** 42 de 88 tarefas concluídas; 46 pendentes
...
Prosseguir com `SIGEC-P5-03`, permitindo que admin/gerente abra somente arquivos tecnicamente validados e limpos, aprove ou rejeite a versão atual com motivo público obrigatório na rejeição e registre nota interna separada. Os testes remotos existentes que reutilizam os mesmos usuários sintéticos devem continuar sequenciais. Para fechar posteriormente o Gate P2, ainda será necessário revisar a prévia real, importar as 364 linhas prontas, corrigir ou excluir as 25 pendentes, finalizar e registrar as seis decisões exigidas pelo gate e confirmar a versão oficial de pontuação/desempates. Até lá, o banco rejeita corretamente a publicação e a classificação automática permanece fechada.
