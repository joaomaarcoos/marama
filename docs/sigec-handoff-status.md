...
- [ ] SIGEC-P1-03 — Implementar CAPTCHA e rate limit por IP, e-mail e telefone. **Implementação técnica concluída em 27/08/2026:** widget Turnstile no cadastro, login e recuperação; token enviado ao Supabase Auth; buckets atômicos com identificadores em HMAC e nonce server-only contra bypass direto. Sitekey, segredo HMAC local, exigência do CAPTCHA no Supabase e gate de prontidão confirmados por teste. **Smoke oficial parcial em 28/08/2026:** widget carregado no login e token aceito pelo Supabase; saúde e guardas de rota aprovadas. O runtime atual já possui as variáveis obrigatórias com comprimentos válidos, mas a recuperação ainda responde indisponível. A correção em validação torna o consumo dos buckets sequencial e registra somente a etapa segura da falha; repetir o smoke após a publicação para concluir.
