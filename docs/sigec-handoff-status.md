...
Prosseguir com `SIGEC-P5-01`, criando a lista administrativa paginada de candidaturas com filtros por processo, município, curso, modalidade, concorrência, status e pendência, sem expor dados pessoais além do necessário para a triagem. Os testes remotos existentes que reutilizam os mesmos usuários sintéticos devem continuar sequenciais. Para fechar posteriormente o Gate P2, ainda será necessário revisar a prévia real, importar as 364 linhas prontas, corrigir ou excluir as 25 pendentes, finalizar e registrar as seis decisões exigidas pelo gate e confirmar a versão oficial de pontuação/desempates. Até lá, o banco rejeita corretamente a publicação e a classificação automática permanece fechada.
