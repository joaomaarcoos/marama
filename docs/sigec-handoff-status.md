...
**Fase atual:** `SIGEC-P5-05` concluída na branch `codex/sigec-p5-05-gate-avanco`; próxima implementação é `SIGEC-P5-06`; Gate P2 permanece adiado pelas confirmações normativas e pela importação oficial
**Progresso auditado:** 45 de 88 tarefas concluídas; 43 pendentes
...
Prosseguir com `SIGEC-P5-06`, modelando os motivos de desclassificação como catálogo versionado e decisão auditável, sem transformar as regras do edital antigo em regra definitiva enquanto a reconfirmação normativa estiver pendente. A interface deve separar motivo público de nota interna e exigir seleção explícita por `admin`/`gerente`. Para fechar posteriormente o Gate P2, ainda será necessário revisar a prévia real, importar as 364 linhas prontas, corrigir ou excluir as 25 pendentes, finalizar e registrar as seis decisões exigidas pelo gate e confirmar a versão oficial de pontuação/desempates. Até lá, o banco rejeita corretamente a publicação e a classificação automática permanece fechada.
