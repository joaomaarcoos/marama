...
def main() -> int:
    ...
    try:
        ...
        reset(cur); cur.execute("select has_function_privilege('authenticated','public.sigec_get_application_submission_readiness(uuid)','EXECUTE'), not has_function_privilege('anon','public.sigec_get_application_submission_readiness(uuid)','EXECUTE')"); expect("least_privilege_grants",cur.fetchone()==(True,True))
