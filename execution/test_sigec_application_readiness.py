...
def main() -> int:
    ...
    try:
        ...
        cur.execute("insert into public.sigec_process_questions(process_id,code,label,question_type,required,config) values(%s,'observacao','Observação adicional','short_text',false,%s) returning id",(process,Json({"audience":"all"}))); other_question=cur.fetchone()[0]
        ...
        cur.execute("insert into public.sigec_document_requirements(process_id,code,label,required,condition_config) values(%s,'complemento','Comprovante complementar',false,%s) returning id",(process,Json({"audience":"all"}))); other_requirement=cur.fetchone()[0]
        ...
        error(cur,"closed_window_blocks_answer_replacement","select public.sigec_replace_application_answers(%s,%s)",(application,Json({str(question):True})),"23514")
        reset(cur); error(cur,"closed_window_blocks_document_without_request","select * from public.sigec_register_candidate_document(%s,%s,%s,'fechado.pdf','application/pdf',10,%s,%s,null)",(application,requirement,f"{candidate}/{application}/{uuid.uuid4()}.pdf","3"*64,candidate),"23514")
        error(cur,"malformed_diligence_fields_rejected","insert into public.sigec_information_requests(application_id,message,requested_fields,due_at,requested_by) values(%s,'Pedido inválido',%s,now()+interval '1 hour',%s)",(application,Json([{"field":"document"}]),manager),"22023")
        requested_fields=Json([{"kind":"question","id":str(question)},{"kind":"document","id":str(requirement)}])
        cur.execute("insert into public.sigec_information_requests(application_id,message,requested_fields,due_at,requested_by) values(%s,'Envie os dados solicitados',%s,now()+interval '1 hour',%s) returning id",(application,requested_fields,manager)); diligence=cur.fetchone()[0]
        auth(cur,other); error(cur,"other_candidate_cannot_answer_diligence","select * from public.sigec_submit_information_request_answers(%s,%s)",(diligence,Json({str(question):True})),"42501")
        auth(cur,candidate); error(cur,"diligence_rejects_unrequested_question","select * from public.sigec_submit_information_request_answers(%s,%s)",(diligence,Json({str(other_question):"fora do escopo"})),"42501")
        cur.execute("select * from public.sigec_submit_information_request_answers(%s,%s)",(diligence,Json({str(question):True}))); expect("requested_answer_allowed_after_deadline",cur.fetchone()==(1,False))
        reset(cur); error(cur,"diligence_rejects_unrequested_document","select * from public.sigec_register_candidate_document(%s,%s,%s,'fora.pdf','application/pdf',10,%s,%s,%s)",(application,other_requirement,f"{candidate}/{application}/{uuid.uuid4()}.pdf","4"*64,candidate,diligence),"42501")
        diligence_path=f"{candidate}/{application}/{uuid.uuid4()}.pdf"; cur.execute("select * from public.sigec_register_candidate_document(%s,%s,%s,'solicitado.pdf','application/pdf',10,%s,%s,%s)",(application,requirement,diligence_path,"5"*64,candidate,diligence)); diligence_document=cur.fetchone()[0]; expect("requested_document_allowed_after_deadline",diligence_document is not None)
        cur.execute("select public.sigec_record_document_malware_scan(%s,%s,'clean','clamav',null,null)",(diligence_document,"5"*64)); cur.execute("select public.sigec_finalize_information_request_if_complete(%s,%s)",(diligence,candidate)); expect("complete_diligence_closes_request",cur.fetchone()[0] is True)
        cur.execute("select status,answered_at is not null from public.sigec_information_requests where id=%s",(diligence,)); expect("diligence_answered_state_recorded",cur.fetchone()==("answered",True))
        auth(cur,candidate); error(cur,"answered_diligence_cannot_be_reused","select * from public.sigec_submit_information_request_answers(%s,%s)",(diligence,Json({str(question):False})),"23514")
        reset(cur); cur.execute("select public.sigec_close_information_request(%s,%s,'accepted','Informações conferidas no teste')",(manager,diligence)); expect("answered_diligence_can_be_accepted",cur.fetchone()[0]=="accepted")
        cur.execute("insert into public.sigec_information_requests(application_id,message,requested_fields,created_at,due_at,requested_by) values(%s,'Pedido expirado',%s,now()-interval '2 hours',now()-interval '1 hour',%s) returning id",(application,Json([{"kind":"document","id":str(requirement)}]),manager)); expired_request=cur.fetchone()[0]
        error(cur,"expired_diligence_cannot_upload","select * from public.sigec_register_candidate_document(%s,%s,%s,'expirado.pdf','application/pdf',10,%s,%s,%s)",(application,requirement,f"{candidate}/{application}/{uuid.uuid4()}.pdf","6"*64,candidate,expired_request),"23514")
        cur.execute("select count(*) from public.sigec_audit_events where entity_id=%s and action in ('information_request_answers_updated','information_request_answered')",(str(diligence),)); expect("diligence_actions_are_audited",cur.fetchone()[0]==2)
        cur.execute("select has_function_privilege('authenticated','public.sigec_get_application_submission_readiness(uuid)','EXECUTE'), not has_function_privilege('anon','public.sigec_get_application_submission_readiness(uuid)','EXECUTE'), has_function_privilege('authenticated','public.sigec_submit_application(uuid,text,text)','EXECUTE'), has_function_privilege('authenticated','public.sigec_start_application_correction(uuid)','EXECUTE'), not has_function_privilege('anon','public.sigec_start_application_correction(uuid)','EXECUTE'), has_function_privilege('authenticated','public.sigec_submit_information_request_answers(uuid,jsonb)','EXECUTE'), not has_function_privilege('anon','public.sigec_submit_information_request_answers(uuid,jsonb)','EXECUTE'), not has_function_privilege('authenticated','public.sigec_finalize_information_request_if_complete(uuid,uuid)','EXECUTE')"); expect("least_privilege_grants",cur.fetchone()==(True,True,True,True,True,True,True,True))
