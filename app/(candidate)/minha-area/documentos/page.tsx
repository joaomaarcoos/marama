...
export default async function CandidateDocumentsPage() {
  ...
  const [{ data: requirementRows }, { data: documentRows }, { data: questionRows }, { data: answerRows }] = await Promise.all([
  ...
  const requirements = (requirementRows || []).map((row: any) => {
    ...
    return { id: row.id, processId: row.process_id, applicableApplicationIds, label: row.label, description: row.instructions || '', required: row.required, mimeTypes: row.accepted_mime_types || [], maxSizeBytes: Number(row.max_file_size_bytes) }
  })
