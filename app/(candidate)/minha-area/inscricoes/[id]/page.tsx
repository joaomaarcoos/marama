...
export default async function CandidateApplicationPage({ params }: { params: { id: string } }) {
  ...
  const [{ data: vacancies }, { data: preferences }, { data: questionRows }, { data: answerRows }, { data: readinessRows }, { data: submissionRows }, { data: requestRows }, { data: requirementRows }, { data: disqualificationRows }, { data: appealWindowRows }] = await Promise.all([
    ...
    adminClient.from('sigec_appeal_windows').select('opens_at,closes_at,timezone').eq('process_id', application.process_id).order('created_at', { ascending: false }).limit(1),
  ])
