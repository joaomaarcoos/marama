...
export default async function CandidateApplicationPage({ params }: { params: { id: string } }) {
  ...
  const [{ data: vacancies }, { data: preferences }, { data: questionRows }, { data: answerRows }, { data: readinessRows }, { data: submission }] = await Promise.all([
    ...
    supabase.from('sigec_application_submissions').select('protocol, submitted_at').eq('application_id', application.id).order('version', { ascending: false }).limit(1).maybeSingle(),
  ])
  ...
  return <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-10"><Link href="/minha-area" className="inline-flex items-center gap-2 text-sm font-bold text-[#526074]"><ArrowLeft className="h-4 w-4" /> Voltar para minha área</Link><header className="mt-7 border-b border-[#d9e0e7] pb-7"><p className="text-xs font-extrabold uppercase tracking-[.18em] text-[#16845f]">Sua inscrição</p><h1 className="mt-2 font-display text-3xl font-bold text-[#172033] sm:text-4xl">Complete sua inscrição</h1><p className="mt-3 text-sm text-[#657084]">{process.title}. Escolha suas vagas e responda às perguntas abaixo.</p></header><div className="mt-8"><CandidateVacancyPreferences applicationId={application.id} vacancies={options} initialIds={(preferences || []).map((item) => item.vacancy_id)} maxPreferences={process.max_preferences} locked={application.application_state !== 'draft'} /></div><CandidateApplicationQuestions applicationId={application.id} questions={questions} initialAnswers={initialAnswers} locked={application.application_state !== 'draft'} />{application.application_state === 'draft' && <CandidateSubmissionReadiness items={readiness} />}<CandidateApplicationSubmit applicationId={application.id} enabled={coreReady} protocol={submission?.protocol} submittedAt={submission?.submitted_at} /></main>
}
