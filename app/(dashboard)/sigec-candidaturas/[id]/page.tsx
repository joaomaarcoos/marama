...
import type { SigecAcademicProductionReview, SigecAcademicProductionScore, SigecAdvancementReadiness, SigecApplicationDetail, SigecConsolidatedScoreSnapshot, SigecDiligenceOption, SigecDisqualificationDecision, SigecDisqualificationReason, SigecDocumentReview, SigecExperienceReview, SigecExperienceScore, SigecInformationRequest, SigecPostgraduateEducation, SigecPostgraduateReview, SigecPostgraduateScore, SigecStageOption, SigecTeachingExperience } from '@/lib/sigec-application-detail'
...
export default async function SigecApplicationDetailPage({ params }: { params: { id: string } }) {
  ...
  const [reviewsResult, requestsResult, questionsResult, requirementsResult, readinessResult, transitionsResult, catalogsResult, decisionResult, educationResult, postgraduateReviewsResult, postgraduateScoreResult, experienceResult, experienceReviewsResult, experienceScoreResult, academicReviewsResult, academicScoreResult, scoreSnapshotResult] = !error && detail && identityResult.data?.candidate_id
    ? await Promise.all([
      ...
      admin.from('sigec_application_score_snapshots').select('id,version,algorithm_version,postgraduate_points,experience_points,academic_points,total_points,created_at').eq('application_id', parsedId.data).order('version', { ascending: false }).limit(1).maybeSingle(),
    ])
    : [{ data: [] }, { data: [] }, { data: [] }, { data: [] }, { data: [] }, { data: [] }, { data: null }, { data: null }, { data: [] }, { data: [] }, { data: [] }, { data: [] }, { data: [] }, { data: [] }, { data: [] }, { data: [] }, { data: null }]
